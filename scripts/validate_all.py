...
def check_anchor_exists(file_path: Path, anchor: str) -> bool:
    """Check if an anchor (id or name attribute) exists in an HTML file"""
    ...
    try:
        ...
        # Check for id="anchor" or name="anchor" or id='anchor' or name='anchor'
        pattern = rf'(?:id|name)\s*=\s*["\']{re.escape(anchor_id)}["\']'
            ...
            for link_text, link_url in md_links:
                ...
                if not link_url.startswith('http'):
                    ...
                    if link_url.startswith('#'):
                        # Markdown anchors are auto-generated by GitHub from headers
                        # Skip validation - they'll work on GitHub
                        result.add_pass("Documentation", f"{md_path.name}: Markdown anchor '{link_text}' -> '{link_url}' (auto-generated)")
                        continue
                    ...
                    if file_part.startswith('./'):
                        link_path = md_path.parent / file_part[2:]
                    elif file_part.startswith('../'):
                        link_path = md_path.parent.parent / file_part[3:]
                    elif file_part:
                        link_path = md_path.parent / file_part
                    else:
                    ...
                    if not link_path.exists():
                        result.add_error("Documentation", f"{md_path.name}: Broken link '{link_text}' -> '{link_url}' (file not found)")
                        continue
                    
                    # If it's a markdown file with anchor, skip anchor validation (auto-generated)
                    if link_path.suffix == '.md' and anchor:
                        result.add_pass("Documentation", f"{md_path.name}: Link '{link_text}' -> '{link_url}' (markdown anchor auto-generated)")
                    # If it's an HTML file with anchor, check if anchor exists
                    elif link_path.suffix == '.html' and anchor:
                        if check_anchor_exists(link_path, anchor):
                            result.add_pass("Documentation", f"{md_path.name}: Link '{link_text}' -> '{link_url}' works")
                        else:
                            result.add_error("Documentation", f"{md_path.name}: Broken anchor '{link_text}' -> '{link_url}' (anchor not found)")
                    else:
                        # File exists, no anchor or anchor not applicable
                        result.add_pass("Documentation", f"{md_path.name}: Link '{link_text}' -> '{link_url}' works")
                else:
    ...
    for html_path in html_files:
        ...
        try:
            ...
            for link_url in href_links:
                # Skip pure anchors (same-page links) and external links
                ...
                # Resolve file path
                if file_part.startswith('./'):
                    link_path = html_path.parent / file_part[2:]
                elif file_part.startswith('../'):
                    link_path = html_path.parent.parent / file_part[3:]
                elif file_part:
                    link_path = html_path.parent / file_part
                else:
                    continue
                
                # Check if file exists
                if not link_path.exists():
                    result.add_error("Documentation", f"{html_path.name}: Broken link '{link_url}' (file not found)")
                    continue
                
                # If it's an HTML file with anchor, check if anchor exists
                if link_path.suffix == '.html' and anchor:
                    if check_anchor_exists(link_path, anchor):
                        result.add_pass("Documentation", f"{html_path.name}: Link '{link_url}' works")
                    else:
                        result.add_error("Documentation", f"{html_path.name}: Broken anchor '{link_url}' (anchor not found)")
                # If it's a markdown file with anchor, skip anchor validation (auto-generated)
                elif link_path.suffix == '.md' and anchor:
                    result.add_pass("Documentation", f"{html_path.name}: Link '{link_url}' (markdown anchor auto-generated)")
                else:
                    # File exists, no anchor or anchor not applicable
                    result.add_pass("Documentation", f"{html_path.name}: Link '{link_url}' works")
